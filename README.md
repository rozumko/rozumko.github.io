...
**Що вже працює сьогодні (реалізоване ядро):** офіційні онлайн-події з кодами
доступу, кабінети вчителя й адміністратора, серверне оцінювання (ключі
відповідей не потрапляють у браузер), браузерні сертифікати/дипломи та
**Шкільний режим** — самостійні місії на `/school` і класна гра в стилі
Kahoot (вчитель створює код, учні приєднуються анонімно з аватаром і
прізвиськом, лідерборд без ПІБ).

| Поверхня | Призначення | Стан |
|---|---|---|
| **Шкільний режим** | Класний режим для вчителів і груп, відокремлений від батьківських оплат і дитячих персональних даних. | ✅ Працює: самостійні місії + класна гра за кодом |
| **Домашні місії** | Батьківський шлях для корисного екранного часу, основ ШІ та поступового прогресу. | 🔵 У планах: немає батьківських акаунтів, оплат, дитячих профілів чи consent (позначки `[PLANNED]` — в [architecture.md](./docs/architecture.md)) |
...
- **Чисте розділення School/Home** (напрям, у розробці) - класна активність не переносить індивідуальні результати дітей у батьківські акаунти.
