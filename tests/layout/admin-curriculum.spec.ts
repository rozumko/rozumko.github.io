...
test('LearningApps toolbar validates links and saves structured exercise content', async ({ page }) => {
  await page.route('https://learningapps.org/**', route => route.fulfill({ contentType: 'text/html', body: '<p>Exercise preview</p>' }))
  const { rows } = await mockCurriculumAdmin(page)
  await openCurriculumTab(page)
  await page.locator('[data-lesson-id="g2-m2-l8"]').getByRole('button', { name: 'Редагувати' }).click()
  const view = editorView(page)
  ...
  const canvas = view.locator('details[data-block-id]').filter({ has: page.getByRole('tab', { name: 'Учитель' }) })
  await canvas.getByRole('tab', { name: 'Презентація' }).click()
  await canvas.getByRole('button', { name: 'Вставити вправу LearningApps' }).click()
  await canvas.getByLabel('Посилання на вправу LearningApps').fill('https://evil.test/view20183559')
  await canvas.getByRole('button', { name: 'Вставити', exact: true }).click()
  await expect(canvas.getByRole('alert')).toContainText('Потрібне посилання LearningApps')
  await canvas.getByLabel('Посилання на вправу LearningApps').fill('https://learningapps.org/view20183559')
  await canvas.getByRole('button', { name: 'Переглянути вправу' }).click()
  await expect(canvas.locator('iframe')).toHaveAttribute('src', 'https://learningapps.org/watch?app=20183559&disableanalytics=1')
  await canvas.getByRole('button', { name: 'Вставити', exact: true }).click()
  await expect(canvas.locator('[data-learningapps-id="20183559"]')).toHaveCount(1)
  await canvas.getByRole('button', { name: 'Початковий код HTML' }).click()
  await canvas.getByRole('button', { name: 'Застосувати HTML до цієї вкладки' }).click()
  await view.getByRole('button', { name: 'Зберегти' }).click()
  await expect(view.locator('.cl-ed-message')).toContainText('Збережено')
  const saved = (rows.get('g2-m2-l8')!.draftContent as any).blocks.find((entry: any) => entry.type === 'canvas')
  expect(saved.content.board).toEqual([{ type: 'learningapps', appId: '20183559' }])
})
