...
test('the standards catalogue shows which skills cover each code and drafts a new skill from one', async ({ page }) => {
  ...
  // An old portal code typed into the mapping resolves to the same entry and is saved as the normative code.
  await modal.getByLabel('Відповідність 1: код або критерій').fill('2 ІФО 3.3.1')
  await expect(modal.locator('.of-ref-hint')).toContainText('Код у стандарті: 2 ІФО 3.3, його й буде збережено.')
  ...
  expect(writes[writes.length - 1]).toMatchObject({
    outcome: { code: 'INF-2-DEV-4', source: 'internal', gradeBand: '1-2', mappings: [{ framework: 'nush-ifo-2018', ref: '2 ІФО 3.3' }] },
  })
  // Linked is not covered: without a direct mapping the entry keeps showing up in «Без прямої відповідності».
  const linked = catalog.locator('[data-ref-code="2 ІФО 3.3"]')
