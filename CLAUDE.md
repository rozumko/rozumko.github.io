...
- Коментарі та ідентифікатори в коді — англійською (кирилиця в файлах ламається при PowerShell-редагуванні); при редагуванні старих файлів українські коментарі можна перекладати. Українська лишається тільки в UI-текстах для дітей/батьків/вчителів
...
backend/src/routes/curriculum-admin.ts ← Lesson Engine editorial API (/api/admin/curriculum, flag LESSON_ENGINE_ENABLED) + subject-packs.ts registry; admin tabs curriculum-tab.ts (editor) + outcomes-tab.ts
...
backend/src/lib/lesson-evidence.ts ← Lesson Engine evidence rows, outcome summary rule, lesson report (/report; report-view.ts); outcomes: DB directory (0057, lib/curriculum-outcomes.ts, admin tab outcomes-tab.ts)
