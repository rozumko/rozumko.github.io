...
Статус оновлено 2026-07-15: базова карта 1–4 класів, батьківські акаунти,
профілі, `client-unverified` path-progress, мікро-уроки, редактор карти та
версіонування ревізій реалізовані. Наступний погоджений продуктовий зріз —
етап 6: retention-пілот відкладеного підтвердження на трьох точках 2 класу.
Checkout, subscription locks і повне curriculum-масштабування залишаються
запланованими після перевірки повернення, навчального ефекту та собівартості.
...
## Етап 6 — Retention-пілот відкладеного підтвердження (погоджено 2026-07-15)

Мета: перевірити не «чи вистачає точок», а чи створює цикл
`Дізнаюсь → Застосовую → Підтверджую` корисну причину повернутися і чи
покращує він збереження навички. Розширена карта до приблизно 40 точок на клас
лишається **curriculum backlog / north star**, а не зобов'язанням виробити
160 повноцінних точок до перевірки цієї гіпотези.

Статуси цього етапу:

- **РЕАЛІЗОВАНО** — підтверджено в поточному коді або даних;
- **ЗАПЛАНОВАНО** — погоджено до виконання; статус змінюється лише після коду,
  тестів і, де потрібно, застосованої міграції/деплою.

### 6.0. Межі й специфікація експерименту

- **РЕАЛІЗОВАНО:** карта 2 класу має 9 точок; батьківські акаунти, верифіковані
  email, профілі дітей і ownership-gated синхронізація path-progress працюють.
- **РЕАЛІЗОВАНО:** результати карти мають `trust: client-unverified` і можуть
  використовуватися як формувальна практика та продуктова телеметрія, але не
  як trusted score, офіційний звіт про опанування чи підстава для диплома.
- **РЕАЛІЗОВАНО:** поточні `bestStars` означають якість виконання сесії, а не
  стадію опанування; точка має лише стан `completed`.
- **ЗАПЛАНОВАНО:** до першого учасника зафіксувати версію пілота, календар,
  baseline, правила включення, формули метрик і go/no-go пороги. Після старту
  не змінювати їх заднім числом; усі зміни оформлювати як нову версію пілота.
- **ЗАПЛАНОВАНО:** пілот вимагає батьківського акаунта: due-планувальник,
  mastery-стани і серверна аналітика працюють лише для дитячих профілів під
  акаунтом. Анонімні/локальні гравці у вибірку не входять і стани
  «повторення скоро / час підтвердити» не бачать. Це обмеження і пов'язаний
  selection bias (діти вже залучених батьків) явно фіксуються у правилах
  включення та в інтерпретації результатів; мінімум профілів з 6.6
  рахується лише в цій підгрупі.
- **ЗАПЛАНОВАНО:** пілот не блокує подальшу карту календарем. Дитина може йти
  вперед; відкладене підтвердження з'являється як окрема бажана ціль без FOMO,
  таймерів втрати нагород або штучного уповільнення.

### 6.1. Обсяг пілота і контент

Пілот охоплює рівно три точки 2 класу:

1. `g2-ct-algorithms` — алгоритми;
2. `g2-digital-safety` — приватні дані й цифрова безпека;
3. `g2-ai-perception` — як ШІ розпізнає об'єкти.

- **РЕАЛІЗОВАНО:** тематичні question-пули мають відповідно 7, 6 і 6 питань;
  поточний picker перемішує весь відповідний пул і не виключає вже показані ID.
- **ЗАПЛАНОВАНО:** створити приблизно 8 нових рецензованих питань на кожну тему
  (орієнтир — 24 нові айтеми). Це перший контрольний замір собівартості
  авторингу, предметної рецензії, реалізації, QA та accessibility.
- **ЗАПЛАНОВАНО:** для кожної теми сформувати два версіоновані паралельні набори
  A/B: `apply` і `confirm`. Вони не перетинаються за `questionId`, але вирівняні
  за `skillKey`, складністю, форматом, кількістю кроків і вимогами до читання.
- **ЗАПЛАНОВАНО:** додати явний споживаний механізм вибору набору
  (`questionSetId`/еквівалент). Серверне виключення показаних ID для
  static-bundle місій недоступне без серверної видачі питань, тож реалістичний
  механізм — `questionSetId` у бандлі + дисципліна вибору на клієнті.
  Просте додавання питань у спільний випадковий пул не приймається.
- **ЗАПЛАНОВАНО:** confirm-набір не розкривається до due: повторний прохід
  точки («переграти») використовує лише apply-набір; confirm-набір видає
  тільки confirm-флоу після `reviewDueAt`. Обидва набори лежать у публічному
  practice-бандлі разом із ключами (свідома властивість practice-режиму),
  тому аналітика веде прапорець контамінації: якщо айтеми confirm-набору
  були показані профілю до due, пара виключається з навчальної метрики.
- **ЗАПЛАНОВАНО:** структурний тест гарантує неперетин A/B, унікальність ID,
  достатній розмір, однакове покриття навичок і валідність кожного айтема.
- **ЗАПЛАНОВАНО:** для `g2-ai-perception` додати короткий рецензований крок
  «Дізнаюсь», якщо на момент пілота в точці немає відповідного мікро-уроку.
- **ЗАПЛАНОВАНО:** для експерименту контрбалансувати A→B і B→A між профілями,
  щоб різниця складності наборів не маскувалась під ефект пам'яті.

### 6.2. Модель опанування і дизайн точки

- **ЗАПЛАНОВАНО:** не перевантажувати поточні performance-зірки новою
  семантикою. Додати окремий `masteryState`:
  `learned → applied → review-due → confirmed`.
- **ЗАПЛАНОВАНО:** зберігати щонайменше `appliedAt`, `reviewDueAt`,
  `confirmedAt`, версію набору та показані `questionId`; схема і API мають бути
  профільними, версіонованими та ідемпотентними.
- **ЗАПЛАНОВАНО:** візуально показати три частини відзнаки опанування:
  «Дізнався», «Застосував», «Підтвердив». Вона має бути відмінною від зірок за
  якість виконання, але третя частина повинна бути помітною і бажаною.
- **ЗАПЛАНОВАНО:** стани вузла карти: доступний, у роботі, застосовано,
  повторення скоро, час підтвердити, підтверджено. Колір не є єдиним способом
  передавання стану; потрібні текст/іконка, aria-label і клавіатурний фокус.
- **ЗАПЛАНОВАНО:** батьківський огляд пояснює стан простою мовою; не називає
  `client-unverified` результат офіційною оцінкою або доведеним «володінням».

### 6.3. Планувальник повторення

- **ЗАПЛАНОВАНО:** у першій версії використовувати фіксований інтервал:
  `reviewDueAt = appliedAt + 72 години`. Адаптивне вікно 2–5 днів — лише після
  пілота, щоб не додавати ще одну змінну.
- **ЗАПЛАНОВАНО:** основне вікно старту рахується від `reviewDueAt`, а не від
  завершення первинної точки: старт confirm протягом 72 годин після due.
  Саме підтвердження не згорає: воно доступне й після вікна, а пізній старт
  просто рахується поза основною метрикою (без штрафів і втрати відзнаки).
- **ЗАПЛАНОВАНО:** на Home-екрані показати чергу «Час повторити» вище швидкої
  практики; підтвердження можна пройти без пошуку старої точки на карті.
- **ЗАПЛАНОВАНО:** повторна доставка due-події та повторний submit не створюють
  дубль, не накручують attempts і не змінюють immutable content version.

### 6.4. Канал повернення

- **ЗАПЛАНОВАНО:** фаза A — in-app due-черга без зовнішнього нагадування. Вона
  перевіряє видимість, старт і завершення повторення серед тих, хто повернувся.
- **ЗАПЛАНОВАНО:** фаза B — один opt-in email батькові після `reviewDueAt`.
  Верифікований auth-email сам по собі не вважається згодою на retention-листи.
- **ЗАПЛАНОВАНО:** email має окрему згоду/вимкнення, ідемпотентний ключ,
  rate limit, журнал `queued/sent/failed`, нейтральний текст без результатів,
  імені чи інших даних дитини та посилання на батьківський Home-вхід.
- **ЗАПЛАНОВАНО:** лист не містить tracking-пікселя; відкриття не
  відстежуються. Ефект email вимірюється лише переходом за посиланням
  (`reminder_clicked` з ідемпотентним токеном) і подальшим стартом confirm.
- **ЗАПЛАНОВАНО:** ефект email аналізується окремо від ефекту механіки. Без
  фази B негативний D7 не можна трактувати як спростування користі повторення.
- **ЗАПЛАНОВАНО:** push-інфраструктура не входить у цей пілот.

### 6.5. Аналітика і навчальний ефект

- **ЗАПЛАНОВАНО:** події щонайменше:
  `apply_completed`, `review_became_due`, `review_queue_viewed`,
  `review_started`, `review_completed`, `reminder_queued`, `reminder_sent`,
  `reminder_failed`, `reminder_clicked`. Події містять профільний псевдонімний
  ID, point/content/set version, source (`in-app|email|direct`), час і агрегат
  результату; не містять текстів відповідей або зайвих персональних даних.
- **ЗАПЛАНОВАНО:** головна навчальна метрика — accuracy на відкладеному
  непересічному confirm-наборі (контаміновані пари з 6.1 виключаються).
  Додатково: `confirm − apply` на вирівняних наборах, розріз за `skillKey`,
  складністю і форматом.
- **ЗАПЛАНОВАНО:** поведінкові метрики: старт confirm протягом 72 годин після
  due, completion серед тих, хто стартував, D7/D30 return і активні дні за
  14/30 днів. Для фази A окремо звітувати умовну метрику серед тих, хто вже
  повернувся; не видавати її за causal retention lift.
- **ЗАПЛАНОВАНО:** виробничі метрики: години педагогічного дизайну, авторингу,
  рецензії, реалізації, QA/accessibility та виправлень на тему й на опубліковану
  apply/confirm-пару.
- **ЗАПЛАНОВАНО:** поточний `client-unverified` контур достатній для
  формувального пілота й агрегованого продуктового рішення. Якщо результат
  пізніше впливає на paid report, диплом або офіційне підтвердження, відповідь
  і ключ переносяться у серверно-довірений scoring flow до такого використання.

### 6.6. Робочі go/no-go критерії

Пороги нижче є стартовими. Їх можна скоригувати після калібрування наборів і
baseline, але треба заморозити до першого учасника пілота.

- **ЗАПЛАНОВАНО — мінімум даних:** щонайменше 30 завершених apply→confirm пар
  від щонайменше 15 різних дитячих профілів; один профіль не може створювати
  більшість вибірки повторними спробами.
- **ЗАПЛАНОВАНО — навчання:** медіанна confirm-accuracy не нижча за 70%, а
  медіанна дельта `confirm − apply` не гірша за −10 відсоткових пунктів на
  контрбалансованих наборах. Якщо поведінка добра, але навчальний критерій не
  пройдено — механіку не масштабувати, спочатку переглянути контент/інтервал.
- **ЗАПЛАНОВАНО — використання:** не менше 70% тих, хто почав confirm,
  завершують його. Окремо показувати частку стартів серед усіх eligible у фазах
  A і B; не змішувати in-app та email в одну цифру.
- **ЗАПЛАНОВАНО — повернення:** фаза B має покращити старт confirm і/або D7
  проти baseline фази A. За малої вибірки рішення спирається на узгоджені
  leading indicators і within-subject порівняння, а не на заяву про
  статистично доведений causal lift.
- **ЗАПЛАНОВАНО — якість:** немає P0/P1 дефектів, порушень ownership/consent,
  дублювання due/email-подій або критичних accessibility-регресій.
- **ЗАПЛАНОВАНО — вартість:** зафіксована повна фактична собівартість трьох
  тем і підтверджено, що темп виробництва вкладається у доступну місячну
  потужність команди. Без цього кількість точок не затверджується.

Масштабування дозволене лише якщо одночасно пройдені навчальний, поведінковий,
якісний і виробничий критерії. Якщо навчальний ефект є, а повернення немає —
переглянути канал/видимість. Якщо повернення є, а навчального ефекту немає —
це «театр повернень», а не підстава розширювати механіку.

### 6.7. Порядок після успішного пілота

- **ЗАПЛАНОВАНО:** перенести mastery-стани й due-чергу на решту шістьох точок,
  щоб ними були охоплені всі 9 точок 2 класу; непересічні набори додавати лише
  після заміру їхньої собівартості.
- **ЗАПЛАНОВАНО:** додати одну AI-вправу «Прийняти / виправити / відхилити
  відповідь ШІ + вибрати причину» поверх наявного патерну `scenarios`.
  Спочатку це `client-unverified` practice; trusted scoring додається тільки
  перед використанням у paid report/дипломі.
- **ЗАПЛАНОВАНО:** лише після перевірки 2 класу затвердити розширений перелік
  точок для 1, 3 і 4 класів та річний контентний ритм.
  Чернетку повної видимої річної послідовності зафіксовано в
  `docs/yearly-home-path-v1.md` як контентний backlog: приблизно 40 точок на
  клас із позначками `ready`, `candidate`, `placeholder`.
- **ЗАПЛАНОВАНО:** не застосовувати симетрію 12/12/12. Частка AI literacy
  зростає з класом; сім'я тем може мати `minGrade/maxGrade` і не зобов'язана
  мати штучну версію для кожного класу.
- **ЗАПЛАНОВАНО:** AILit/Cambridge/НУШ та інші framework mappings зберігати як
  змінювані метадані/доказове зіставлення, а не як назви дитячих точок або
  жорстку структуру карти. На першому зрізі додавати лише поля з реальним
  споживачем (`skillKey`, progression/set metadata); не будувати повну
  таксономію наперед.
- **ЗАПЛАНОВАНО:** розрізняти в cost-моделі: (A) новий контент у наявній
  механіці; (B) параметрична item-модель; (C) нова механіка; (D) редактор або
  відкритий цифровий продукт. AIG використовувати там, де шаблон окупається
  (трасування, послідовності, дані); етику й рефлексію не примушувати до
  генератора, якщо ручні рецензовані варіанти дешевші й якісніші.
...
- ~~Міграції 0027/0028 на проді~~ — звірено 2026-07-10: схема актуальна до 0028
  ...
  застосуванням.
- ✅ Міграцію lead → акаунт для наявних лідів вирішено через явний
  `/api/parent/claim-lead`: parent auth + чинний lead-token + збіг
  підтвердженого email (див. етап 3.4).
...
- ✅ Попередній порядок виконано до adult registration gate і підтвердженого
  перенесення лише домашнього локального прогресу включно.
- Наступний порядок: етап 6.0–6.6 на трьох точках 2 класу → go/no-go рішення →
  етап 6.7 для всіх 9 точок 2 класу → лише потім curriculum-масштабування та
  subscription locks після окремого checkout-рішення.
