...
- ✅ Зріз 4a (2026-07-14): структура шляху в БД — таблиця `path_maps`
  ...
  лишився в CI.
- Зріз 4b: формовий редактор точок в адмінці (не візуальний граф) з
  живим прев'ю карти; автоінкремент version кроку при зміні складу;
  перехід клієнта на бандл `public/path/` з фолбеком на вбудовану копію.
